# Hexo Blog Editor - Техническая Спецификация

## Обзор Проекта

**Название:** Hexo Blog Editor (HexTool)
**Тип:** Desktop GUI приложение для редактирования Hexo блогов
**Целевая аудитория:** Пользователи Hexo, желающие иметь визуальный редактор вместо работы с markdown файлами напрямую
...
#[tauri::command]
fn get_project_config(project_path: String) -> Result<HexoConfig, String>
...
// Hexo команды
#[tauri::command]
fn run_hexo_command(project_path: String, command: String) -> Result<CommandOutput, String>

#[tauri::command]
fn start_hexo_server(project_path: String) -> Result<String, String>

#[tauri::command]
fn stop_hexo_server(server_id: String) -> Result<(), String>
...
struct Post {
    id: String,              // Путь к файлу относительно source/_posts/
    title: String,
...
struct ImageInfo {
    filename: String,
    path: String,           // Относительный путь от source/images/
...
**Функциональность:**
- Просмотр всех изображений из `source/images/`
...
- Автоматическое копирование изображений в `source/images/`
...
**Описание:** Запуск Hexo команд из GUI.

**Функциональность:**
- Запуск локального сервера (`hexo server`)
  - Индикатор статуса (запущен/остановлен)
  - Отображение URL (http://localhost:4000)
  - Кнопка "Открыть в браузере"
  - Кнопка "Остановить сервер"
- Генерация статики (`hexo generate`)
  - Progress bar
  - Логи выполнения
...
**Описание:** Выбор папки с Hexo проектом при запуске приложения.

**Функциональность:**
- При первом запуске - диалог выбора папки
- Валидация выбранной папки:
  - Проверка наличия `_config.yml`
  - Проверка наличия `source/` папки
  - Проверка наличия `package.json` с hexo
...
- Создание поста:
  ...
  - Создание файла в `source/_posts/`
...
- Создание страницы:
  ...
  - Создание `index.md`
- Создание черновика:
  - Аналогично посту, но в `source/_drafts/`
...
#### Week 4: Hexo Integration + Project Selection
- [ ] Выбор папки проекта
- [ ] Валидация Hexo проекта
- [ ] Запуск Hexo команд (server, generate, deploy)
...
1. **Нет работы с темами Hexo**
   - Редактор не управляет темами, только контентом

2. **Нет поддержки плагинов Hexo**
   - Только базовая функциональность Hexo
...
- Поддержка 99% Hexo markdown синтаксиса
...
Данная спецификация описывает полнофункциональный GUI редактор для Hexo блогов на базе Tauri + Svelte. Проект фокусируется на минималистичном UX, производительности и кроссплатформенности.
...
- Hexo интеграция (server, generate, deploy)
